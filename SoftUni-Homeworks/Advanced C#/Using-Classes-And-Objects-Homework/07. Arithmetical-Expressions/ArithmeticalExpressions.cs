...
{
    // we define 3 types of input: arithmetic operations symbols, verbal arithmetic operations signs, and brackets
    ...
    // the code below creates a string list of values and arithmetic operations symbols
    ...
    // setting arithmetic operations precedence
    ...
    public static Queue<string> ConvertToRPN(List<string> tokens)
    {
        ...
        for (int i = 0; i < tokens.Count; i++)
        {
            ...
            else if (currentToken == ",")
            {
                ...
                while (stack.Peek() != "(") // stack.Peek returns the object at the top of the Stack without removing it
                {
                    string currentElement = stack.Pop();
                    queue.Enqueue(currentElement);
                }
            }
            //check brackets positions
    ...
    public static double GetResultRPN(Queue<string> queue)
    {
        ...
        while (queue.Count != 0)
        {
            ...
            {
                if (currentToken == "+") // addition requires at least two values
                {
                    if (stack.Count < 2)
                    {
                        throw new ArgumentException("Invalid expression!");
                    }
                    // if we have the two values available - we remove them, and replace them with firstValue + secondValue
    ...
    static void Main()
    {
        ...
        try
        {
            Console.WriteLine("Arithmetical expression to solve: ");
            string input = "pow(2, 3.14) * (3 - (3 * sqrt(2) - 3.2) + 1.5*0.3)"; //Console.ReadLine().Trim();
