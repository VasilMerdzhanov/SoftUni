...
class NumberCalculations
{
    static void Main()
    {
        ...
        Console.WriteLine("Please, enter a couple of floating-point numbers, all in one line, separated by a space: ");
        ...
        Console.WriteLine("\nPlease, enter a couple of type int numbers, all in one line, separated by a space: ");
        int[] integers = Console.ReadLine().Split(charSeparators, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();

        Console.WriteLine("0utput:\nMin: {0}, Max: {1}, Sum: {2}, Average: {3}, Product: {4}",
            GetMin(integers), GetMax(integers), GetSum(integers), GetAverage(integers), GetProduct(integers));
    }
    // Min
